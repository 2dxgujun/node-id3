...
NodeID3.prototype.getFrameSize = function(buffer, decode) {
  if (decode) {
    return this.decodeSize(
      new Buffer([buffer[6], buffer[7], buffer[8], buffer[9]])
    )
  } else {
...
NodeID3.prototype.createPictureFrame = function(data) {
  try {
    ...
    let buff = Buffer.concat([bHeader, bContent, apicData])
    return buff
  } catch (e) {
    return e
  }
...
NodeID3.prototype.readPictureFrame = function(APICFrame) {
  let picture = {}
  let APICMimeType = APICFrame.toString('ascii').substring(
    1,
  ...
  if (APICFrame[0] == 0x00) {
    ...
    if (descFound) {
      picture.description =
        iconv.decode(desc.slice(0, descFound), 'utf16') || undefined
    }
  }
  if (descEnd) {
...
NodeID3.prototype.createContentDescriptor = function(
  ...
  return terminated
    ? Buffer.concat([
        description,
        new Buffer(this.getTerminationCount(encoding)).fill(0x00)
      ])
    : description
}
